...
def main():
    calibImages = [cv2.imread(f'dataset/cam2/{i}.png') for i in range(len(os.listdir('dataset/cam2/')))]
    objectPoints, imgPoints = getObjAndImgPoints(calibImages, (height, width), lenCell, show=True)
    mtx, dist = getCalibrationData(calibImages[0], objectPoints, imgPoints)
    print(f'Mtx:\n{mtx}')
    print(f'Dist:\n{dist}')

    img = cv2.imread('dataset/cam2/5.png')
